...
function processResult(lines, dayOnly) {
  var today = new Date();
  var date_now_f = today.getFullYear() + '-' + ('0' + (today.getMonth() + 1)).slice(-2) + '-' + ('0' + today.getDate()).slice(-2);
  ...
  for (i = 0; i < lines.length; i++) {
    ...
    if (lines[i].includes('DTSTART') && HAR_HITTAT_VEVENT) {
      ...
      var date_f = date_tot[0].slice(0, 4) + '-' + date_tot[0].slice(4, 6) + '-' + date_tot[0].slice(6, 8);
  ...
  else { var final = "NO_EVENTS" };
  return final;
};

module.exports = function () {
  this.getCalStr = function (debugMode, dayOnly, callback) {
    var request = new XMLHttpRequest();
    url = debugMode ? "https://www.calendarlabs.com/ical-calendar/ics/71/Sweden_Holidays.ics" : "https://www.nordiskamuseet.se/calendar/ical/ical/calendar-nordiska-museet.ics";
    request.open(
      "GET",
      url,
      true
    ...
    request.onreadystatechange = function () {
      ...
      if (request.readyState === 4 && request.status === 200) {
        ...
        if (type.indexOf("text") !== 1) {
          ...
          // this is where we process the XMLHttpRequest
          final = processResult(lines, dayOnly)
          if (callback) return callback(final);
